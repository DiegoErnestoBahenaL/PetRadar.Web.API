...
namespace PetRadar.Web.API.IntegrationTests.Tests
{
    ...
    public class LoginControllerTests : IClassFixture<TestingWebAppFactory>
    {
        ...
        public async Task Login_Returns_Token_Succesfully()
        {
            ...
            // Act
            var result = await _client.PostAsync("/api/gate/login", new StringContent(jsonModel, Encoding.UTF8, MediaTypeNames.Application.Json));
        ...
        public async Task RefreshToken_Returns_New_Token_Succesfully()
        {
            ...
            // Act
            var result = await _client.PostAsync("/api/gate/login/refresh", new StringContent(jsonModel, Encoding.UTF8, MediaTypeNames.Application.Json));
        ...
        {
            // Arrange
            var refreshTokenModel = new RefreshTokenFromUiModel(null);
