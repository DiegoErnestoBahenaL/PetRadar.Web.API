...
namespace PetRadar.Core.Domain.Models
{
    public class UserCreateModel
    {
        ...
        [StringLength(255, MinimumLength = 1, ErrorMessage = "The field Email is not valid, check the format"), Required, EmailAddress]
        public string Email { get; set; } = string.Empty;

        [StringLength(255, MinimumLength = 1, ErrorMessage = "The field Password must be in range of {2} and {1}"), Required]
        public string Password { get; set; } = string.Empty;

        [StringLength(255, MinimumLength = 1, ErrorMessage = "The field Name must be a string with a minimum length of {2} and a maximum length of {1}"), Required]
        ...
        public UserCreateModel(string email, string password, string name, string? lastName,
            string? phoneNumber, string? organizationName, string? organizationAddress, string? organizationPhone,
            RoleEnum role)
        {
            Email = email;
            Password = password;
